...
class ReceiptPosition:
    def __init__(self, item: Item, number_of_items: int = 0, weight: Weight = None):
        self._item = item
    ...
    def get_amount(self):
        return self._weight if self._weight is not None else self._number_of_items
...
class Receipt:
    ...
    def add_position(
        ...
        weight = Weight(weight_value) if weight_value else None
        new_position = ReceiptPosition(item, number_of_items, weight)
        if new_position not in self._positions:
            self._positions.append(new_position)

    def remove_position(self, name):
        dummy_position = ReceiptPosition(Item(name, Price(0)), 1)
        self._positions = [x for x in self._positions if not x == dummy_position]
